...
思路：
    N的取值范围是 [1, 30]，意味着K的取值可能达到230，这将是一个非常大的数，因此想构造出这个序列然后直接找出第K未是非常不现实的。

    暴力：我们完全按照题目描述中的指示处理每一行，而只需要保存最新的那一行。不幸的是，字符串的长度可能有 10 亿左右，因为每一行的
		长度都是前一行的两倍，所以这种方法不够高效。
			时间复杂度：O(2^N)，我们解析每一行所需要的时间和其长度有关，共计 2^0 + 2^1 + ... + 2^{N-1}
			空间复杂度：O(2^N)，最后一行（lastrow）的长度。

    递归：(K + 1) / 2是K的父节点所在的序号。如果K是奇数，则K的状态与K的父节点的状态相同，否则K的状态与K的父节点状态相反。
	...
	递归（父变体）：因为生成每一行只需要前一行的信息，所以我们可以考虑解析前一行的位来输出答案。来看这个例子，如果我们中间有一行是 "0110"，
	    ...
	    父变体1：第 K 位的父位是第 (K+1) / 2 位。如果父位是 0，那么这一位就是 1 - (K % 2)。如果父位是 1，那么这一位就是 K % 2。
		父变体2：第 K 位的父位是第 (K+1) / 2 位。如果这一位是偶数（K % 2 == 0）。如果父位是 1，那么这一位就是 0。反之为 1
